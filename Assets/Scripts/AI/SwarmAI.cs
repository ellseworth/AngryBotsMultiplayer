...
public class SwarmAI : MonoBehaviour
{
	...
	public float zapDist = 2.3f, slowDownDist = 1.0f, rechargeDist = 5.5f, chargeTime = 6.0f, visibleChargeFraction = 0.8f,
		nonAttackSpeedFactor = 0.2f, damageAmount = 5.0f, minTimeBetweenAttacks = 1.0f;
	public AudioClip zapSound, rechargeSound;
	...
	private void Awake ()
	{
		...
		for (int i = 0; i < buzzers.Length; i++)
		{
			...
			if (!buzzers[i]) Debug.Log ("buzzer not found at "+i, transform);	
	...
	private void OnTriggerEnter (Collider other)
	{
		...
		{
			attacking = true;
			for (int i = 0; i < buzz.Count; i++) buzz[i].motor.enabled = true;
		}
	}
	
	private void OnTriggerExit (Collider other)
	{
		if (other.transform == player) attacking = false;
	}
	...
	private void Update ()
	{
		for (int c = buzz.Count - 1; c >= 0; c--)
		{
			...
			{
				buzz.RemoveAt (c);
				if (buzz.Count > 0) attackerIndex = attackerIndex % buzz.Count;
			}
		}
		if (buzz.Count == 0) return;
	...
	private void UpdateRetreat ()
	{
		for (int i = 0; i < buzz.Count; i++)
		{
			if (buzz[i].motor.enabled)
			{
				...
				if (spawnDir.sqrMagnitude > 1) spawnDir.Normalize ();
	...
	private void UpdateAttack ()
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (i == attackerIndex)
			{
				...
				{
					if (playerDist < rechargeDist) buzz[i].motor.movementDirection = playerDir * -nonAttackSpeedFactor;
					else buzz[i].motor.movementDirection = Vector3.zero;
				}
				else
				{
					...
					if (playerDist < zapDist + slowDownDist)
					{
						...
						if (playerDist < zapDist && aimingCorrect)
						{
							...
							if (targetHealth) targetHealth.OnDamage (damageAmount, -playerDir);
			...
			else
			{
				...
				Vector3 posSide =
					player.position + Vector3.Project (-playerDir * playerDist, attackerDir) + fleeDir * s * rechargeDist;
				...
				if (buzz[i].charged == 1) pos += Vector3.up * 2;
				buzz[i].motor.movementDirection = (pos - buzz[i].transform.position).normalized * nonAttackSpeedFactor;
				if ((i+1) % count == attackerIndex) buzz[i].motor.facingDirection = playerDir;
				else buzz[i].motor.facingDirection = buzz[i].motor.movementDirection;
			}
			
			// Recharge
			buzz[i].charged += Time.deltaTime / chargeTime;
			if (buzz[i].charged > 1) buzz[i].charged = 1;
			...
			buzz[i].motor.GetComponent<Rigidbody>().angularVelocity += Random.onUnitSphere * 4 * visibleCharged;
		}
	}
	
	private IEnumerator DoElectricArc (BuzzerData buzz)
	{
		// Play attack sound
