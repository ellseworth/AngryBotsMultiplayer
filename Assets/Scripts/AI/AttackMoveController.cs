...
public class AttackMoveController : MonoBehaviour
{
	...
	public float targetDistanceFront = 2.5f, targetDistanceBack = 1.0f;
	...
	private float goingRight = 1, directionChangeTime = 0;
	...
	{
		if (weaponBehaviour) weaponBehaviour.enabled = true;
	}

	private void OnDisable()
	{
		if (weaponBehaviour) weaponBehaviour.enabled = false;
	}
	...
	private void Update()
	{
		...
		if (degreesFromPlayerForward < 120) targetPosition += character.right * goingRight;
		...
		if (targetVector.sqrMagnitude > 1) targetVector.Normalize();
