...
public class PatrolRoute : MonoBehaviour
{
	...
	public int GetClosestPatrolPoint (Vector3 pos)
	{
		...
		if (!pingPong || shortestIndex < patrolPoints.Count - 1)
		{
			...
			if (angle > 120) shortestIndex = nextIndex;
		}
	...
	private void OnDrawGizmos ()
	{
		...
		if (pingPong) loopCount--;
		for (int i = 0; i < loopCount; i++)
		{
			if (!patrolPoints[i]) break;
	...
	public int GetIndexOfPatrolPoint (PatrolPoint point)
	{
		for (int i = 0; i < patrolPoints.Count; i++)
		{
			if (patrolPoints[i] == point) return i;
		}
		return -1;
	}
	...
	public GameObject InsertPatrolPointAt (int index)
	{
		...
		else
		{
			...
			{
				go.transform.position =
					(patrolPoints[count-1].transform.position * 2 - patrolPoints[count-2].transform.position);
			}
