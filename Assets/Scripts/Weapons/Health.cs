...
{
	public float maxHealth = 100.0f, health = 100.0f, regenerateSpeed;
	...
	public GameObject scorchMarkPrefab, scorchMark;

	public SignalSender damageSignals, dieSignals;

	private float lastDamageTime;
	private ParticleEmitter damageEffect;
	private float damageEffectCenterYOffset, colliderRadiusHeuristic = 1.0f;
	...
	private void Awake()
	{
		...
		if (damagePrefab)
		{
			if (damageEffectTransform == null) damageEffectTransform = transform;
		...
		if (scorchMarkPrefab) {
			scorchMark = Instantiate(scorchMarkPrefab, Vector3.zero, Quaternion.identity) as GameObject;
		...
		*/
		...
		if (damageEffect)
		{
			...
			if (!damageEffectCentered)
			{
				...
				damageEffect.transform.position =
					(transform.position + Vector3.up * damageEffectCenterYOffset) + colliderRadiusHeuristic * dir;
			}
		...
		if (health <= 0)
		{
			...
			if (scorchMark) {
				...
				scorchMark.transform.eulerAngles = new Vector3(
					scorchMark.transform.eulerAngles.x, Random.Range(0.0f, 90.0f), scorchMark.transform.eulerAngles.z
					);
            }
		}
	}
